...
export class ComprehensiveAgentCatalog {
  ...
  public static initializeAgentCatalog(): void {
    ...
    // Cache full catalog for performance
    this.cacheFullCatalog();
  }

  /**
   * Cache full agent catalog in Redis
   */
  private static async cacheFullCatalog(): Promise<void> {
    try {
      const catalogData = {
        configs: Array.from(this.agentConfigs.entries()),
        totalCount: this.agentConfigs.size,
        cachedAt: new Date().toISOString()
      };
      await cacheService.cacheAgentCatalog(catalogData);
    } catch (error) {
      // Silently fail if Redis unavailable - caching is optional
    }
  }
  ...
  /**
   * Get agent configuration by ID (with Redis caching)
   */
  public static async getAgentConfig(agentId: string): Promise<AgentConfig | undefined> {
    // Try cache first
    try {
      const cached = await cacheService.getCachedAgentConfig(agentId);
      if (cached) return cached;
    } catch (error) {
      // Continue to fallback if cache fails
    }
    ...
    if (config) {
      try {
        await cacheService.cacheAgentConfig(agentId, config);
      } catch (error) {
  ...
  /**
   * Get agents by tier (with Redis caching)
   */
  public static async getAgentsByTier(tier: AgentTier): Promise<AgentConfig[]> {
    // Try cache first
    try {
      const cached = await cacheService.getCachedAgentsByTier(tier);
      if (cached) return cached;
    } catch (error) {
      // Continue to fallback if cache fails
    }
    ...
    // Fallback to in-memory
    const agents = Array.from(this.agentConfigs.values()).filter(config => config.tier === tier);
    
    // Cache for future requests
    try {
      await cacheService.cacheAgentsByTier(tier, agents);
    } catch (error) {
      // Silently fail - caching is optional
    }
  ...
  /**
   * Get agents by specialization (with Redis caching)
   */
  public static async getAgentsBySpecialization(specialization: string): Promise<AgentConfig[]> {
    // Try cache first
    try {
      const cached = await cacheService.getCachedAgentsBySpecialization(specialization);
      if (cached) return cached;
    } catch (error) {
      // Continue to fallback if cache fails
    }
    ...
    // Fallback to in-memory
    const agents = Array.from(this.agentConfigs.values()).filter(config => config.specialization === specialization);
    
    // Cache for future requests
    try {
      await cacheService.cacheAgentsBySpecialization(specialization, agents);
    } catch (error) {
      // Silently fail - caching is optional
    }
  ...
  /**
   * Get agent statistics (with Redis caching)
   */
  public static async getAgentStatistics(): Promise<Record<string, any>> {
    // Try cache first
    try {
      const cached = await cacheService.getCachedAgentStatistics();
      if (cached) return cached;
    } catch (error) {
      // Continue to fallback if cache fails
    }
    ...
    // Cache for future requests
    try {
      await cacheService.cacheAgentStatistics(stats);
    } catch (error) {
      // Silently fail - caching is optional
    }
...
export { ComprehensiveAgentCatalog };