...
export class PartPackerFlow extends EventEmitter {
  ...
  private generateContextualColor(style?: string): [number, number, number] {
    const styleColors = {
      modern: [0.9, 0.9, 0.9],
      industrial: [0.5, 0.5, 0.6],
      organic: [0.8, 0.7, 0.6],
      futuristic: [0.2, 0.8, 1.0]
    };
    
    return styleColors[style as keyof typeof styleColors] || [Math.random(), Math.random(), Math.random()];
  }
  ...
  private generateTriangles(count: number): number[] {
    const triangles: number[] = [];
    for (let i = 0; i < count; i++) {
...
export class PartPackerMaster3D extends EventEmitter {
  ...
  private async processModelToSTL(model: PartPacker3DModel, options: any): Promise<ArrayBuffer> {
    ...
    return new TextEncoder().encode(stlContent).buffer;
  }
...
<model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02">
  ...
  private async processModelToOBJ(model: PartPacker3DModel, options: any): Promise<ArrayBuffer> {
    ...
    return new TextEncoder().encode(objContent).buffer;
  }
  ...
  private generateMeshXML(part: PartPacker3DPart, index: number): string {
    ...
    const triangles = part.geometry.triangles.map((face: number[]) => `${face[0]} ${face[1]} ${face[2]}`).join(' ');
  ...
  private optimizeTriangles(triangles: number[]): number[] {
